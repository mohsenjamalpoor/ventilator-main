...
const COLOR_WARN = "#F59E0B"; // amber — caution
const COLOR_GOOD = "#34D399"; // emerald — relevance / evidence
const COLOR_BAD = "#FB7185"; // rose — elevated / risk
const COLOR_INFO = "#38BDF8"; // sky — measurement / reference
  ...
  return (
    <svg viewBox="0 0 480 170" className="w-full h-auto">
      ...
      <line
        ...
        strokeDasharray="6 4"
      />
      <text x="360" y="155" fontSize="11" fill={COLOR_INFO}>
        سطح PEEP
      </text>
    </svg>
  );
                ...
                <tbody>
                  {TARGETS.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: COLOR }}>
                        {row.plateau}
                      </td>
                      <td className="px-4 py-3" style={{ color: COLOR }}>
                        {row.driving}
